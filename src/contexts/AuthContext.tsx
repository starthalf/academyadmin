...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        console.log('[AUTH] getSession done, session user:', data.session?.user?.id);
  ...
  const loadTeacherAndAcademy = async (authUserId: string) => {
    console.log('[LOAD] teachers query start, authUserId:', authUserId);
    
    // 안전장치: 10초 timeout
    const timeout = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('teachers query timeout 10s')), 10000)
    );
    ...
    try {
      ...
      console.log('[LOAD] teachers query done', { teacherRow, teacherErr });

      if (teacherErr || !teacherRow) {
        console.log('[LOAD] no teacher row found');
        setTeacher(null);
        setAcademy(null);
        return;
      }
      ...
      console.log('[LOAD] academies query start, academyId:', t.academyId);
      ...
      console.log('[LOAD] academies query done', academyRow);
